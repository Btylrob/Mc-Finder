...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Conv2D(
        32, (3, 3),
        activation='relu'),  # applies 32 different filters 3x3 filters 
    tf.keras.layers.MaxPooling2D(
    ),  # picking the most prominent distinct features from our previous layer
    tf.keras.layers.Conv2D(64, (3, 3), activation='relu'),
    tf.keras.layers.MaxPooling2D(),
    tf.keras.layers.Flatten(
    ),  # Flattens pooled map into a single vector for processing
    tf.keras.layers.Dropout(0.5),
    tf.keras.layers.Dense(
        128, activation='relu'
    ),  # makes accurate decision based on previous findings
    tf.keras.layers.Dense(
        len(class_names),
        activation='softmax')  # turns vectors into a probability distroution 
])
...
#train model
history = model.fit(data, validation_data=val_data,
                    epochs=10)  # 10 complete cycles through data
...
with gr.Blocks(title="🍔 Mc-Finder") as demo:
    gr.Markdown(
        "## 🍔 Mc-Finder\nUpload a food image to identify the McDonald's item!")

    with gr.Row():
        with gr.Column(scale=1):
            inp = gr.Image(type="pil", label="Upload an Image")
        with gr.Column(scale=1):
            out = gr.Textbox(label="🍟 Predicted Class", interactive=False)
    ...
    # Flagging section
    with gr.Row():
        flag_btn = gr.Button("Flag Result")
        flag_btn.click(lambda *args: callback.flag(list(args)), [inp, out],
                       None,
                       preprocess=False)
    ...
    gr.Markdown("### 🔍 Try Examples Below:")
    gr.Examples(
        examples=
        [[
         ...
         ], ["testing-data/images (8).jpeg"], ["testing-data/images.jpeg"]],
        inputs=inp,
    )
